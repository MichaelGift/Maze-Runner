 ...
 *
 * @angle: The angle of the ray relative to the player's view.
 *
 * Return: void
 */
...
/**
 * vertIntersect - Performs vertical ray casting and determines
 * if the ray intersects with a vertical wall.
 *
 * @angle: The angle of the ray relative to the player's view.
 *
 * Return: void
...
/**
 * castRay - Casts a ray at a specified angle and determines
 * the distance to the nearest wall hit (horizontal or vertical).
 *
 * @angle: The angle of the ray relative to the player's view.
 * @id: The identifier of the ray being cast.
 *
 * Return: void
 */
...
/**
 * castRays - Casts rays from the player's viewpoint
 * across the projection plane.
 *
 * Each ray is cast at a specific angle relative
 * to the player's rotation angle.
 *
 * The function iterates over all rays and calculates
 * their angles before calling castRay.
 *
 * Return: void
 */
void castRays(void)
{
	...
	{
		float rayAngle =
			player.rotationAngle + atan((ray - NUM_RAYS / 2) / PROJ_PLANE);
 ...
 * by drawing lines from the player's position to each ray's wall
 * hit position.
 *
 * The function iterates over rays with a step of 50 to draw line
